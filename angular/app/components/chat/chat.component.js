class ChatController {
  ...
  openChanel(userInfo){
    ...
    this.API.one('message','message-with').get({with_id: this.openChanelId}).then((response) => {
  ...
  sendMessage(message)
  {
    if(message == '')
      return
    this.message = ''
    let newMessage = {to_id:this.openChanelId, message: message}

    this.API.all('message/create').post(newMessage).then((response) => {
        this.addMessage({message:message, to_id:this.openChanelId, user:this.userInfo})
    })
  }
}
